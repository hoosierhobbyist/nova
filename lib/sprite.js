...
var DIE = new Symbol('DIE');
var WRAP = new Symbol('WRAP');
var STOP = new Symbol('STOP');
var SPRING = new Symbol('SPRING');
var BOUNCE = new Symbol('BOUNCE');
    ...
    function Sprite() {
        ...
        if (config['radius'] != null) {
            __edge = new _geometry.Circle(config['radius'], this);
                ...
                var pts = config['points'].map(function (pt) {
        ...
        value: function start() {
            ...
            if (!this.isRunning) {
                this.updateID = setInterval(function () {
                    _this.update();
                }, 1000 / _engine2['default'].frameRate);
            } //end if
        }
        //end ::start
...
_engine2['default'].on('start', Sprite.start);
_engine2['default'].on('stop', Sprite.stop);
